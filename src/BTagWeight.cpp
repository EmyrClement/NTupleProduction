...
std::vector<double> BjetWeights(const pat::JetCollection& jets, unsigned int numberOfBtags, std::string MCSampleTag) {
	...
	boost::scoped_ptr<BTagWeight> btagWeight(new BTagWeight(MCSampleTag));
...
BTagWeight::BTagWeight(std::string MCSampleTag) :
				minNumberOfTags_(0), //
				maxNumberOfTags_(0),
				MCSampleTag_(MCSampleTag){
}

double BTagWeight::weight(const pat::JetCollection& jets) const {
...
std::vector<double> BTagWeight::getScaleFactor( const double partonFlavour, const pat::Jet& jet ) const {
...
std::vector<double> BTagWeight::getBScaleFactor(const pat::Jet& jet, double uncertaintyFactor) const {
	double SFb(0);
	double sf_error(0);
	...
	if ( MCSampleTag_ == "Summer12" ) { // 2012 btag scale factors
	...
	else if ( MCSampleTag_ == "Summer11Leg" ) { // 2011 btag scale factors
...
std::vector<double> BTagWeight::getCScaleFactor(const pat::Jet& jet) const { // doesn't seem to be used anywhere??
	// Same in both 8TeV and 7TeV
	return getBScaleFactor(jet, 2.0);
}

std::vector<double> BTagWeight::getUDSGScaleFactor(const pat::Jet& jet) const {
	double pt = jet.pt();
	double eta = abs(jet.eta());
	double SF_udsg_mean(0), SF_udsg_min(0), SF_udsg_max(0);
	std::vector<double> SF_udsg_and_error;

	if ( MCSampleTag_ == "Summer12" ) { // 2012
		if (pt < 20) {
	...
	} else if ( MCSampleTag_ == "Summer11Leg" ) { // 2011
		if (pt < 20) {
			SF_udsg_and_error.push_back(0.);
			SF_udsg_and_error.push_back(0.);
			return SF_udsg_and_error;
...
double BTagWeight::getMinUDSGScaleFactor(double jetPT, double jetEta) const {
	...
	if ( MCSampleTag_ == "Summer11Leg" ) { //2011
		if (jetPT > 670 && jetEta >=0. && jetEta <=2.4) {
			// Integrated over eta
...
double BTagWeight::getEfficiency( const unsigned int partonFlavour, const pat::Jet& jet ) const {
	if ( partonFlavour == 5) { //b-quark
...
double BTagWeight::getBEfficiency(const pat::Jet& jet) const {
...
double BTagWeight::getCEfficiency(const pat::Jet& jet) const {
	std::vector<double> binEdges{ 20, 30, 40, 50, 60, 70, 80, 100, 120, 160, 210, 260, 320, 400, 500, 600, 800 };
	const double jetPt = jet.pt();
...
double BTagWeight::getUDSGEfficiency(const pat::Jet& jet) const {
	std::vector<double> binEdges{ 20, 30, 40, 50, 60, 70, 80, 100, 120, 160, 210, 260, 320, 400, 500, 600, 800 };
	const double jetPt = jet.pt();
